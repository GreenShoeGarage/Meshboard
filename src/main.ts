import "./styles.css";
import { APP_VERSION, defaultSettings, emptyProject, normalizeProject, type AnalyticsTimeRange, type AppSettings, type EvidenceRecord, type Finding, type MessageDeliveryState, type MessageRecord, type NetworkSnapshot, type NodeColumnId, type NodeMetadata, type NodeObservation, type NodeRecord, type Project, type RuntimeState, type SavedNodeView, type ViewId } from "./models";
import { createDemoProject } from "./demo";
import { deleteProject, estimateStorage, loadCurrentProject, loadSettings, saveProject, saveSettings } from "./storage";
import { MeshtasticAdapter } from "./meshtastic-adapter";
import { csv, downloadBlob, escapeHtml as e, fmtAge, fmtTime, median, nodeId, safeObject } from "./utils";
import { NODE_COLUMNS, appendNodeObservation, filteredSortedNodes, metadataFor, metricStats, nodeMetricSeries, statusForNode, svgSparkline } from "./node-intelligence";
import { filteredPackets, hexLines, portNumStats, relatedPackets } from "./packet-lab";
import { analyticsStats, metricDescriptor, packetRateBins, rangeLabel, rfNodeStats, rfPoints, svgHistogram, svgPacketRate, svgTimeSeries, telemetryKinds, telemetryMetricKeys, telemetryNumericValues, telemetryPoints } from "./rf-telemetry";

const app = document.querySelector<HTMLDivElement>("#app")!;
const fallback = document.querySelector<HTMLDivElement>("#startup-fallback");
if (!app) throw new Error("Application mount point #app was not found.");
...
function addTimeline(type: string, text: string, severity: "INFO"|"LOW"|"MEDIUM"|"HIGH"|"CRITICAL" = "INFO", nodeNum?: number, provenance: "OBSERVED"|"CALCULATED"|"INFERRED"|"USER ENTERED"|"CONFIGURED" = "OBSERVED"): void {
  project.timeline.push({ id: crypto.randomUUID(), time: now(), type, severity, nodeNum, text, provenance });
  if (project.timeline.length > 10000) project.timeline.splice(0, project.timeline.length - 10000);
}

const adapter = new MeshtasticAdapter({
  connection(state, reason) {
    const previous = runtime.connection;
    runtime.connection = state; runtime.connectionReason = reason; runtime.stateChangedAt = now();
    if (state === "CONNECTED") runtime.connectedAt = now();
    if (state === "DISCONNECTED") runtime.disconnectedAt = now();
    if (state === "CONNECTED" && previous !== "CONNECTED") addTimeline("radio connected", "Meshtastic radio synchronized and ready.");
    if (state === "DISCONNECTED" && previous !== "DISCONNECTED" && reason) addTimeline("radio disconnected", reason);
    if (state === "RECOVERING" && previous !== "RECOVERING") addTimeline("radio recovery", reason || "Recovering unexpected radio disconnect.", "LOW");
    scheduleSave(); queueRender();
  },
  progress(progress) { runtime.sync = progress; queueRender(); },
  diagnostics(update) {
    if (update.serialInfo) runtime.serialInfo = { ...runtime.serialInfo, ...update.serialInfo };
  ...
  nodes(nodes) {
    ...
    const latestPackets = new Map<number, typeof project.packets[number]>();
    for (const packet of project.packets) if (packet.direction === "RX" && packet.source !== undefined) latestPackets.set(packet.source, packet);
    ...
    for (const n of project.nodes) appendNodeObservation(project, {
    ...
    evaluateFindings(); scheduleSave(); queueRender();
  },
  ...
  radio(radio) {
    ...
    project.radio = { ...project.radio, ...defined }; scheduleSave(); queueRender();
  },
  packet(packet) {
    project.packets.push(packet);
    if (project.packets.length > settings.livePacketLimit * 5) project.packets.splice(0, project.packets.length - settings.livePacketLimit * 5);
    if (runtime.packetLivePaused && runtime.packetPauseAt && new Date(packet.time).getTime() > new Date(runtime.packetPauseAt).getTime()) runtime.packetNewWhilePaused++;
    ...
    if (packet.direction === "RX" && packet.source !== undefined && (typeof packet.rssi === "number" || typeof packet.snr === "number")) {
      ...
      if (node) {
        if (typeof packet.rssi === "number") node.rssi = packet.rssi;
        if (typeof packet.snr === "number") node.snr = packet.snr;
  ...
  message(message) {
    ...
    if (existing) Object.assign(existing, message, { state: existing.state === "SENDING" ? existing.state : message.state });
    else project.messages.push(message);
    if (project.messages.length > 10000) project.messages.splice(0, project.messages.length - 10000);
    const target = existing || message;
    if (target.packetId !== undefined) {
      const packet = project.packets.find(p => p.packetId === target.packetId);
      if (packet) target.packetRecordId = packet.id;
    }
  ...
  telemetry(reading) {
    ...
    if (project.telemetry.length > settings.telemetryLimit) project.telemetry.splice(0, project.telemetry.length - settings.telemetryLimit);
    const num = (key:string): number|undefined => { const value=reading.values[key]; return typeof value === "number" && Number.isFinite(value) ? value : undefined; };
    const battery=num("batteryLevel") ?? num("battery"); const voltage=num("voltage"); const channelUtilization=num("channelUtilization"); const airUtilTx=num("airUtilTx");
    const node=project.nodes.find(n=>n.num===reading.nodeNum);
    if(node){ if(battery!==undefined)node.battery=battery;if(voltage!==undefined)node.voltage=voltage;if(channelUtilization!==undefined)node.channelUtilization=channelUtilization;if(airUtilTx!==undefined)node.airUtilTx=airUtilTx; }
    if([battery,voltage,channelUtilization,airUtilTx].some(v=>v!==undefined)) appendNodeObservation(project,{id:crypto.randomUUID(),nodeNum:reading.nodeNum,time:reading.time,kind:"TELEMETRY",battery,voltage,channelUtilization,airUtilTx,telemetryRecordId:reading.id,provenance:"OBSERVED"},settings.nodeHistoryLimit);
    scheduleSave(); queueRender();
  },
  position(position) {
    project.positions.push(position);
    if (project.positions.length > 10000) project.positions.splice(0, project.positions.length - 10000);
    const n = project.nodes.find(x => x.num === position.nodeNum);
    if (n) { n.latitude = position.latitude; n.longitude = position.longitude; n.altitude = position.altitude; }
  ...
  activity(direction) {
    if (direction === "RX") { runtime.rxCount++; rxPulse = true; setTimeout(()=>{rxPulse=false;queueRender();},220); }
    else { runtime.txCount++; txPulse = true; setTimeout(()=>{txPulse=false;queueRender();},220); }
  },
  sdkState(state) { runtime.sdkState = state; queueRender(); },
  error(kind, error) {
    if (kind === "decode") runtime.decodeErrors++; else runtime.protocolErrors++;
...
function evaluateFindings(): void {
  const manual = project.findings.filter(f => !f.id.startsWith("AUTO-"));
  const auto: Finding[] = [];
  for (const n of project.nodes) {
    if (typeof n.battery === "number" && n.battery < 20) auto.push({ id:`AUTO-BATT-${n.num}`, title:"LOW BATTERY", severity:"HIGH", confidence:"HIGH", nodeNum:n.num, observedValue:`${n.battery.toFixed(0)}%`, threshold:"< 20%", firstObserved:now(), lastObserved:now(), status:"OPEN" });
    const status = statusForNode(n, settings);
    if (status === "LOST") auto.push({ id:`AUTO-STALE-${n.num}`, title:"STALE NODE", severity:"MEDIUM", confidence:"HIGH", nodeNum:n.num, observedValue:fmtAge(n.lastHeard), threshold:`> ${settings.lostMinutes} minutes`, firstObserved:now(), lastObserved:now(), status:"OPEN" });
  }
  const previous = new Map(project.findings.map(f => [f.id, f]));
  project.findings = [...manual, ...auto.map(f => {
    const old = previous.get(f.id); return old ? { ...f, firstObserved: old.firstObserved, status: old.status, notes: old.notes } : f;
  })];
}
...
function scheduleSave(): void {
  ...
  saveTimer = window.setTimeout(async () => {
    runtime.saveState = "saving"; queueRender();
    try { await saveProject(project); runtime.saveState = "saved"; }
    catch (err) { console.error(err); runtime.saveState = "error"; }
    queueRender();
  }, 350);
}
...
function navItems(): Array<[ViewId,string,string,boolean]> {
  return [
...
function viewHtml(view: ViewId): string {
  switch(view) {
    case "home": return homeView(); case "messages": return messagesView(); case "nodes": return nodesView(); case "map": return mapView();
    case "topology": return topologyView(); case "rf": return rfView(); case "packets": return packetsView(); case "telemetry": return telemetryView();
    case "radio": return radioView(); case "channels": return channelsView(); case "configuration": return configView(); case "timeline": return timelineView();
    case "logbook": return logbookView(); case "compare": return compareView(); case "evidence": return evidenceView(); case "diagnostics": return diagnosticsView();
    case "settings": return settingsView(); case "help": return helpView();
  }
}

function section(title: string, subtitle: string, body: string, actions=""): string {
...
function card(label:string,value:unknown,detail="", cls=""):string { return `<div class="metric-card ${cls}"><span>${e(label)}</span><strong>${e(value ?? "—")}</strong><small>${e(detail)}</small></div>`; }
function nodeName(num?: number): string { if (num === undefined) return "—"; const n=project.nodes.find(x=>x.num===num); return n ? `${n.shortName} · ${n.longName}` : nodeId(num); }
function pct(v?:number):string { return typeof v === "number" ? `${v.toFixed(0)}%` : "—"; }
function fixed(v?:number,d=1,s=""):string { return typeof v === "number" ? `${v.toFixed(d)}${s}` : "—"; }
const BROADCAST_NUM = 0xffffffff;

function conversationKeyForMessage(message: MessageRecord): string {
...
function parseConversation(key: string): { kind:"channel"; channel:number } | { kind:"direct"; peer:number } {
...
function channelName(index:number): string { return project.channels.find(c=>c.index===index)?.name || (index===0?"Primary":`Channel ${index}`); }
...
function conversationMessages(key:string): MessageRecord[] { return project.messages.filter(m=>conversationKeyForMessage(m)===key); }
...
function markConversationRead(key:string): void {
  project.messaging.readAt[key]=now();
  project.messaging.lastConversation=key;
}
function setConversation(key:string, markRead=true): void {
  runtime.selectedConversation=key;
  runtime.selectedMessage=undefined;
  project.messaging.lastConversation=key;
  const conv=parseConversation(key);
  if(conv.kind==="channel") { runtime.messageDestination="broadcast"; runtime.messageChannel=conv.channel; }
  else {
    runtime.messageDestination=conv.peer;
    const recent=[...conversationMessages(key)].reverse().find(m=>Number.isFinite(m.channel));
    if(recent) runtime.messageChannel=recent.channel;
  }
  if(markRead) markConversationRead(key);
  scheduleSave();
}
function messagePacket(message:MessageRecord) {
...
function linkPacketToMessage(packetRecordId:string, packetId?:number): void {
  if(packetId===undefined) return;
  const message=project.messages.find(m=>m.packetId===packetId);
  if(message) message.packetRecordId=packetRecordId;
}
function applyMessageState(message:MessageRecord,state:MessageDeliveryState,reason?:string): void {
  message.state=state;
  if(state==="ACKNOWLEDGED") { message.acknowledgedAt=now(); message.failureReason=undefined; message.failedAt=undefined; }
  if(state==="FAILED") { message.failedAt=now(); message.failureReason=reason || message.failureReason || "Message send failed."; }
}
function draftForConversation(key=runtime.selectedConversation): string { return project.messaging.drafts[key] || ""; }
function setDraftForConversation(text:string,key=runtime.selectedConversation): void {
  project.messaging.drafts[key]=text;
  scheduleSave();
}
function messageStateClass(state:MessageDeliveryState): string { return state.toLowerCase(); }
function messageStateText(message:MessageRecord): string {
  if(message.state==="ACKNOWLEDGED") return "ACKNOWLEDGED";
  if(message.state==="SENDING") return "SENDING / AWAITING ACK";
  if(message.state==="FAILED") return `FAILED${message.failureReason?` · ${message.failureReason}`:""}`;
  if(message.state==="RECEIVED") return "RECEIVED";
  return "STATE UNKNOWN";
}

function connectionProgressPanel(): string {
  if (!["CONNECTING","SERIAL_OPEN","SYNCHRONIZING","RECOVERING","RECONNECTING"].includes(runtime.connection)) return "";
  const steps = [
  ...
  const rank: Record<string,number> = { CONNECTING:0, RECONNECTING:0, RECOVERING:0, SERIAL_OPEN:1, SYNCHRONIZING:2, CONNECTED:3 };
...
function homeView(): string {
  ...
  const snrs = rx.map(p=>p.snr).filter((v):v is number=>typeof v === "number");
...
function messagesView(): string {
  ...
  if (!runtime.selectedConversation) runtime.selectedConversation = project.messaging.lastConversation || "channel:0";
...
function nodeMetadata(nodeNum:number): NodeMetadata | undefined { return project.nodeMetadata.find(x=>x.nodeNum===nodeNum); }
function nodeHasFieldData(nodeNum:number): boolean { const m=nodeMetadata(nodeNum); return !!m && Object.entries(m).some(([k,v])=>k!=="nodeNum"&&k!=="updatedAt"&&v!==undefined&&v!==""); }
function nodeColumnWidth(id:NodeColumnId): number { return project.nodeTable.columnWidths[id] ?? NODE_COLUMNS.find(c=>c.id===id)?.defaultWidth ?? 100; }
function nodeCell(id:NodeColumnId,n:NodeRecord): string {
...
function nodeHistoryCard(nodeNum:number,metric:"rssi"|"snr"|"battery"|"voltage",label:string,unit:string,decimals:number): string {
  const series=nodeMetricSeries(project,nodeNum,metric); const stats=metricStats(series);
...
function nodeInspector(selected:NodeRecord): string {
...
function nodesView(): string {
  ...
  const roles=[...new Set(project.nodes.map(n=>n.role).filter((x):x is string=>!!x))].sort();
  const hardware=[...new Set(project.nodes.map(n=>n.hardware).filter((x):x is string=>!!x))].sort();
  const columns=project.nodeTable.visibleColumns.length?project.nodeTable.visibleColumns:["status","node","lastHeard","battery","rssi","snr"] as NodeColumnId[];
...
function mapView(): string {
  ...
  if (!nodes.length) return section("MAP", "Privacy-preserving local position plot", `<div class="empty large-empty"><b>NO POSITION DATA</b><span>Positions received from Meshtastic nodes will appear here. This local map uses a local coordinate plot and never sends coordinates to a tile service.</span></div>`);
  const lats=nodes.map(n=>n.latitude!), lons=nodes.map(n=>n.longitude!); let minLat=Math.min(...lats), maxLat=Math.max(...lats), minLon=Math.min(...lons), maxLon=Math.max(...lons); if(minLat===maxLat){minLat-=.001;maxLat+=.001} if(minLon===maxLon){minLon-=.001;maxLon+=.001}
  const W=1000,H=620,pad=60; const point=(n:NodeRecord)=>{const x=pad+((n.longitude!-minLon)/(maxLon-minLon))*(W-2*pad); const y=H-pad-((n.latitude!-minLat)/(maxLat-minLat))*(H-2*pad); return {x,y};};
...
function analyticsRangeOptions(selected: AnalyticsTimeRange): string {
  return (["1h","6h","24h","7d","all"] as AnalyticsTimeRange[]).map(v=>`<option value="${v}" ${selected===v?"selected":""}>${rangeLabel(v)}</option>`).join("");
}

function rfView(): string {
...
function packetsView(): string {
  ...
  const portOptions=[...new Set(project.packets.map(p=>p.portNum).filter((x):x is string=>!!x))].sort().map(x=>`<option value="${e(x)}" ${filter.portNum===x?"selected":""}>${e(x)}</option>`).join("");
  const channelOptions=[...new Set(project.packets.map(p=>p.channel).filter((x):x is number=>typeof x==="number"))].sort((a,b)=>a-b).map(x=>`<option value="${x}" ${filter.channel===String(x)?"selected":""}>${x} · ${e(channelName(x))}</option>`).join("");
...
function telemetryView(): string {
  ...
  if(!metrics.some(m=>m.key===state.telemetryMetric)) state.telemetryMetric=metrics[0]?.key||"batteryLevel";
  const descriptor=metricDescriptor(state.telemetryMetric);
  const points=telemetryPoints(project,state.telemetryMetric,range,kind,selectedNode); const stats=analyticsStats(points);
  const readings=project.telemetry.filter(t=>{if(selectedNode!==undefined&&t.nodeNum!==selectedNode)return false;if(kind!=="all"&&t.kind!==kind)return false;const start=range==="all"?undefined:Date.now()-({"1h":3600000,"6h":21600000,"24h":86400000,"7d":604800000} as Record<string,number>)[range]!;return start===undefined||new Date(t.time).getTime()>=start;});
  const reportingNodes=new Set(readings.map(t=>t.nodeNum));
  const latestByKindNode=new Map<string,typeof project.telemetry[number]>();for(const t of readings)latestByKindNode.set(`${t.nodeNum}:${t.kind}`,t);
  ...
  const catalogByKind=new Map<string,Set<string>>();for(const t of readings){const set=catalogByKind.get(t.kind)??new Set<string>();for(const k of Object.keys(telemetryNumericValues(t)))set.add(k.split(".").at(-1)||k);catalogByKind.set(t.kind,set);}
...
function compareView(): string {
  const snaps=project.snapshots; const a=snaps.at(-2), b=snaps.at(-1);
...
function kv(values: Record<string,unknown>): string { return `<dl class="kv">${Object.entries(values).map(([k,v])=>`<div><dt>${e(k)}</dt><dd>${e(v??"—")}</dd></div>`).join("")}</dl>`; }
function objectGrid(value: Record<string,unknown>): string { return `<div class="object-grid">${Object.entries(value).slice(0,16).map(([k,v])=>`<div><span>${e(k)}</span><b>${e(typeof v==="object"?JSON.stringify(v):v)}</b></div>`).join("")}</div>`; }

function bindRenderedControls(): void {
  for (const el of app.querySelectorAll<HTMLElement>("[data-view]")) el.onclick=()=>{runtime.view=el.dataset.view as ViewId; render();};
  for (const el of app.querySelectorAll<HTMLElement>("[data-node]")) el.onclick=()=>{runtime.selectedNode=Number(el.dataset.node); runtime.view="nodes"; render();};
  for (const el of app.querySelectorAll<HTMLElement>("[data-packet]")) el.onclick=()=>{runtime.selectedPacket=el.dataset.packet; render();};
  for (const el of app.querySelectorAll<HTMLElement>("[data-conversation]")) el.onclick=()=>{setConversation(el.dataset.conversation || "channel:0"); render();};
  for (const el of app.querySelectorAll<HTMLElement>("[data-message]")) el.onclick=()=>{runtime.selectedMessage=el.dataset.message; render();};
  for (const el of app.querySelectorAll<HTMLButtonElement>("[data-finding]")) el.onclick=()=>{const f=project.findings.find(x=>x.id===el.dataset.finding); if(f){f.status=f.status==="OPEN"?"ACKNOWLEDGED":f.status==="ACKNOWLEDGED"?"DISMISSED":"OPEN";scheduleSave();render();}};
  for (const el of app.querySelectorAll<HTMLElement>("[data-action]")) el.onclick=()=>void handleAction(el.dataset.action||"");
  const composer=app.querySelector<HTMLTextAreaElement>("#composer-text");
  if(composer) {
    composer.oninput=()=>{setDraftForConversation(composer.value); const c=app.querySelector<HTMLElement>(".byte-count"); const b=new TextEncoder().encode(composer.value).length; if(c){c.textContent=`${b}/228 bytes`;c.classList.toggle("over",b>228);} const send=app.querySelector<HTMLButtonElement>('[data-action="send-message"]'); if(send) send.disabled=runtime.connection!=="CONNECTED"||b===0||b>228;};
    composer.onkeydown=(ev)=>{if(ev.key==="Enter"&&(ev.ctrlKey||ev.metaKey)){ev.preventDefault();void sendMessage();}};
  }
  const channel=app.querySelector<HTMLSelectElement>("#message-channel"); if(channel) channel.onchange=()=>{runtime.messageChannel=Number(channel.value);if(runtime.messageDestination==="broadcast")setConversation(`channel:${runtime.messageChannel}`);scheduleSave();render();};
  const dest=app.querySelector<HTMLSelectElement>("#message-destination"); if(dest) dest.onchange=()=>{runtime.messageDestination=dest.value==="broadcast"?"broadcast":Number(dest.value);setConversation(runtime.messageDestination==="broadcast"?`channel:${runtime.messageChannel}`:`direct:${runtime.messageDestination}`);render();};
  const ms=app.querySelector<HTMLInputElement>("#message-search"); if(ms) ms.oninput=()=>{runtime.messageSearch=ms.value;setTimeout(()=>render(),120);};
  const mf=app.querySelector<HTMLSelectElement>("#message-state-filter"); if(mf) mf.onchange=()=>{runtime.messageStateFilter=mf.value as RuntimeState["messageStateFilter"];render();};
  const ns=app.querySelector<HTMLInputElement>("#node-search"); if(ns) ns.oninput=()=>{project.nodeTable.filter.search=ns.value;project.nodeTable.selectedViewId=undefined;scheduleSave();setTimeout(()=>render(),150);};
  const status=app.querySelector<HTMLSelectElement>("#node-status-filter"); if(status) status.onchange=()=>{project.nodeTable.filter.status=status.value as typeof project.nodeTable.filter.status;project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  const role=app.querySelector<HTMLSelectElement>("#node-role-filter"); if(role) role.onchange=()=>{project.nodeTable.filter.role=role.value;project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  const hw=app.querySelector<HTMLSelectElement>("#node-hardware-filter"); if(hw) hw.onchange=()=>{project.nodeTable.filter.hardware=hw.value;project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  const fav=app.querySelector<HTMLInputElement>("#node-favorites-filter"); if(fav) fav.onchange=()=>{project.nodeTable.filter.favoritesOnly=fav.checked;project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  const view=app.querySelector<HTMLSelectElement>("#node-saved-view"); if(view) view.onchange=()=>{applySavedNodeView(view.value);render();};
  for(const el of app.querySelectorAll<HTMLInputElement>("[data-node-column]")) el.onchange=()=>{const id=el.dataset.nodeColumn as NodeColumnId;const set=new Set(project.nodeTable.visibleColumns);if(el.checked)set.add(id);else set.delete(id);project.nodeTable.visibleColumns=NODE_COLUMNS.map(c=>c.id).filter(id=>set.has(id));if(!project.nodeTable.visibleColumns.length)project.nodeTable.visibleColumns=["node"];project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  for(const el of app.querySelectorAll<HTMLButtonElement>("[data-node-sort]")) el.onclick=(ev)=>{ev.stopPropagation();const id=el.dataset.nodeSort as NodeColumnId;if(project.nodeTable.filter.sortBy===id)project.nodeTable.filter.sortDir=project.nodeTable.filter.sortDir==="asc"?"desc":"asc";else{project.nodeTable.filter.sortBy=id;project.nodeTable.filter.sortDir=id==="node"||id==="hardware"||id==="role"||id==="status"?"asc":"desc";}project.nodeTable.selectedViewId=undefined;scheduleSave();render();};
  for(const el of app.querySelectorAll<HTMLElement>("[data-col-resize]")) el.onpointerdown=(ev)=>{ev.preventDefault();ev.stopPropagation();const id=el.dataset.colResize as NodeColumnId;const startX=ev.clientX,startWidth=nodeColumnWidth(id);const move=(moveEv:PointerEvent)=>{const width=Math.max(60,Math.min(520,startWidth+moveEv.clientX-startX));project.nodeTable.columnWidths[id]=Math.round(width);project.nodeTable.selectedViewId=undefined;for(const cell of app.querySelectorAll<HTMLElement>(`[data-col-cell="${id}"]`)){cell.style.width=`${width}px`;cell.style.minWidth=`${width}px`;}};const up=()=>{document.removeEventListener("pointermove",move);document.removeEventListener("pointerup",up);scheduleSave();};document.addEventListener("pointermove",move);document.addEventListener("pointerup",up,{once:true});};
  const ps=app.querySelector<HTMLInputElement>("#packet-search"); if(ps) ps.oninput=()=>{runtime.packetSearch=ps.value;project.packetLab.filter.search=ps.value;scheduleSave();setTimeout(()=>render(),150);};
  const packetSelect=(id:string,key:"direction"|"portNum"|"channel"|"wantAck"|"encryption"|"timeRange")=>{const el=app.querySelector<HTMLSelectElement>(`#${id}`);if(el)el.onchange=()=>{(project.packetLab.filter as any)[key]=el.value;scheduleSave();render();};};
  packetSelect("packet-direction","direction");packetSelect("packet-portnum","portNum");packetSelect("packet-channel","channel");packetSelect("packet-ack","wantAck");packetSelect("packet-encryption","encryption");packetSelect("packet-time","timeRange");
  const packetText=(id:string,key:"source"|"destination")=>{const el=app.querySelector<HTMLInputElement>(`#${id}`);if(el)el.oninput=()=>{project.packetLab.filter[key]=el.value;scheduleSave();setTimeout(()=>render(),150);};};
  packetText("packet-source","source");packetText("packet-destination","destination");
  for(const el of app.querySelectorAll<HTMLButtonElement>("[data-port-filter]"))el.onclick=()=>{project.packetLab.filter.portNum=el.dataset.portFilter||"";scheduleSave();render();};
  for(const el of app.querySelectorAll<HTMLButtonElement>("[data-packet-tab]"))el.onclick=()=>{project.packetLab.inspectorTab=el.dataset.packetTab as typeof project.packetLab.inspectorTab;scheduleSave();render();};
  for(const el of app.querySelectorAll<HTMLButtonElement>("[data-related-packet]"))el.onclick=()=>{runtime.selectedPacket=el.dataset.relatedPacket;render();};
  for(const el of app.querySelectorAll<HTMLButtonElement>("[data-related-message]"))el.onclick=()=>{runtime.selectedMessage=el.dataset.relatedMessage;const m=project.messages.find(x=>x.id===runtime.selectedMessage);if(m){setConversation(conversationKeyForMessage(m),false);runtime.view="messages";}render();};
  const analyticsRange=app.querySelector<HTMLSelectElement>("#analytics-range");if(analyticsRange)analyticsRange.onchange=()=>{project.rfTelemetry.timeRange=analyticsRange.value as AnalyticsTimeRange;scheduleSave();render();};
  const rfNode=app.querySelector<HTMLSelectElement>("#rf-node-filter");if(rfNode)rfNode.onchange=()=>{project.rfTelemetry.rfNode=rfNode.value;scheduleSave();render();};
  const telemetryNode=app.querySelector<HTMLSelectElement>("#telemetry-node-filter");if(telemetryNode)telemetryNode.onchange=()=>{project.rfTelemetry.telemetryNode=telemetryNode.value;scheduleSave();render();};
  const telemetryKind=app.querySelector<HTMLSelectElement>("#telemetry-kind-filter");if(telemetryKind)telemetryKind.onchange=()=>{project.rfTelemetry.telemetryKind=telemetryKind.value;project.rfTelemetry.telemetryMetric="";scheduleSave();render();};
  const telemetryMetric=app.querySelector<HTMLSelectElement>("#telemetry-metric");if(telemetryMetric)telemetryMetric.onchange=()=>{project.rfTelemetry.telemetryMetric=telemetryMetric.value;scheduleSave();render();};
  bindSettings();
}
...
async function handleAction(action:string): Promise<void> {
  try {
    switch(action){
      case "connection": if(["CONNECTED","CONNECTING","SERIAL_OPEN","SYNCHRONIZING","RECOVERING","RECONNECTING"].includes(runtime.connection)) await adapter.disconnect(); else await adapter.connect(); break;
      case "connect": await adapter.connect(); break;
      case "disconnect": await adapter.disconnect(); break;
      case "reconnect": await adapter.reconnect(); break;
      case "resync": await adapter.resync(); break;
      case "mode-easy": settings.mode="easy";saveSettings(settings);render();break;
      case "mode-advanced": settings.mode="advanced";saveSettings(settings);render();break;
      case "load-demo": if(adapter.active) await adapter.disconnect(); project=createDemoProject();runtime.rxCount=0;runtime.txCount=0;runtime.selectedNode=undefined;runtime.selectedPacket=undefined;runtime.selectedMessage=undefined;runtime.packetLivePaused=false;runtime.packetPauseAt=undefined;runtime.packetNewWhilePaused=0;runtime.packetSearch=project.packetLab.filter.search;runtime.selectedConversation=project.messaging.lastConversation||"channel:0";setConversation(runtime.selectedConversation,false);evaluateFindings();await saveProject(project);render();break;
      case "fresh-start": await freshStart(); break;
      case "import-project": app.querySelector<HTMLInputElement>("#import-file")?.click();break;
      case "export-project": exportProject();break;
      case "send-message": await sendMessage();break;
      case "retry-message": await retryMessage();break;
      case "close-message-inspector": runtime.selectedMessage=undefined;render();break;
      case "open-message-packet": openMessagePacket();break;
      case "export-messages": exportMessages();break;
      case "close-inspector": runtime.selectedNode=undefined;runtime.selectedPacket=undefined;render();break;
      case "save-node-notes": saveNodeFieldData();break;
      case "save-node-field-data": saveNodeFieldData();break;
      case "message-node": openSelectedNodeConversation();break;
      case "node-packets": openSelectedNodePackets();break;
      case "evidence-from-node": evidenceFromNode();break;
      case "clear-node-filters": project.nodeTable.filter={search:"",status:"all",role:"",hardware:"",favoritesOnly:false,sortBy:"lastHeard",sortDir:"desc"};project.nodeTable.selectedViewId=undefined;scheduleSave();render();break;
      case "save-node-view": saveNodeView();break;
      case "delete-node-view": deleteNodeView();break;
      case "export-nodes":exportNodes();break;
      case "packet-live-toggle": if(runtime.packetLivePaused){runtime.packetLivePaused=false;runtime.packetPauseAt=undefined;runtime.packetNewWhilePaused=0;}else{runtime.packetLivePaused=true;runtime.packetPauseAt=now();runtime.packetNewWhilePaused=0;}render();break;
      case "clear-packet-filters": project.packetLab.filter={search:"",direction:"all",portNum:"",channel:"",source:"",destination:"",wantAck:"all",encryption:"all",timeRange:"all"};runtime.packetSearch="";scheduleSave();render();break;
      case "export-filtered-packets": exportFilteredPackets();break;
      case "export-selected-packet": exportSelectedPacket();break;
      case "copy-packet-hex": await copySelectedPacketHex();break;
      case "export-packets":downloadBlob(`${slug(project.name)}-packets.csv`,"text/csv",csv(project.packets.map(p=>safeObject({...p,raw:undefined,decoded:undefined}))));break;
      case "add-log":addLog();break;
      case "snapshot":createSnapshot();break;
      case "evidence-from-packet":evidenceFromPacket();break;
      case "export-rf-analytics":exportRfAnalytics();break;
      case "evidence-from-rf":evidenceFromRf();break;
      case "export-telemetry-analytics":exportTelemetryAnalytics();break;
      case "evidence-from-telemetry":evidenceFromTelemetry();break;
      case "export-diagnostics":await exportDiagnostics();break;
      case "print-report":printReport();break;
      case "clear-session":project.packets=[];project.messages=[];project.telemetry=[];project.positions=[];project.nodeObservations=[];runtime.selectedMessage=undefined;runtime.selectedPacket=undefined;runtime.packetLivePaused=false;runtime.packetPauseAt=undefined;runtime.packetNewWhilePaused=0;runtime.rxCount=0;runtime.txCount=0;addTimeline("session cleared","Operator cleared recorded traffic while preserving project identity and configuration.","INFO",undefined,"USER ENTERED");scheduleSave();render();break;
      case "reset-ui":settings={...defaultSettings};saveSettings(settings);render();break;
    }
  } catch(error){ runtime.connection = adapter.connected ? runtime.connection : "ERROR"; runtime.connectionReason=error instanceof Error?error.message:String(error); addTimeline("application error",runtime.connectionReason,"MEDIUM");render(); }
}

function bindSettings(): void {
  const theme=app.querySelector<HTMLSelectElement>("#setting-theme"); if(theme) theme.onchange=()=>{settings.theme=theme.value as AppSettings["theme"];saveSettings(settings);render();};
  const active=app.querySelector<HTMLInputElement>("#setting-active"); if(active) active.onchange=()=>{settings.activeMinutes=Math.max(1,Number(active.value)||10);settings.staleMinutes=Math.max(settings.activeMinutes+1,settings.staleMinutes);settings.lostMinutes=Math.max(settings.staleMinutes+1,settings.lostMinutes);saveSettings(settings);evaluateFindings();render();};
  const stale=app.querySelector<HTMLInputElement>("#setting-stale"); if(stale) stale.onchange=()=>{settings.staleMinutes=Math.max(settings.activeMinutes+1,Number(stale.value)||30);settings.lostMinutes=Math.max(settings.staleMinutes+1,settings.lostMinutes);saveSettings(settings);evaluateFindings();render();};
  const lost=app.querySelector<HTMLInputElement>("#setting-lost"); if(lost) lost.onchange=()=>{settings.lostMinutes=Math.max(settings.staleMinutes+1,Number(lost.value)||180);saveSettings(settings);evaluateFindings();render();};
  const packets=app.querySelector<HTMLInputElement>("#setting-packets"); if(packets) packets.onchange=()=>{settings.livePacketLimit=Math.max(100,Math.min(10000,Number(packets.value)||2000));saveSettings(settings);render();};
  const tele=app.querySelector<HTMLInputElement>("#setting-telemetry"); if(tele) tele.onchange=()=>{settings.telemetryLimit=Math.max(100,Math.min(50000,Number(tele.value)||5000));saveSettings(settings);render();};
  const history=app.querySelector<HTMLInputElement>("#setting-node-history"); if(history) history.onchange=()=>{settings.nodeHistoryLimit=Math.max(1000,Math.min(250000,Number(history.value)||50000));saveSettings(settings);if(project.nodeObservations.length>settings.nodeHistoryLimit)project.nodeObservations.splice(0,project.nodeObservations.length-settings.nodeHistoryLimit);scheduleSave();render();};
  const sw=app.querySelector<HTMLInputElement>("#setting-sw"); if(sw) sw.onchange=()=>{settings.serviceWorker=sw.checked;saveSettings(settings);void configureServiceWorker();};
  const importFile=app.querySelector<HTMLInputElement>("#import-file"); if(importFile) importFile.onchange=()=>void importProject(importFile.files?.[0]);
}
...
async function sendMessage(): Promise<void> {
  ...
  if(bytes>228)throw new Error(`Message is ${bytes} bytes; maximum safe text payload is 228 bytes.`);
  if(runtime.connection!=="CONNECTED")throw new Error("Connect and synchronize a radio before sending.");
  const my=project.radio.nodeNum??0;
  const destination=runtime.messageDestination;
  const message:MessageRecord={id:crypto.randomUUID(),time:now(),sentAt:now(),from:my,to:destination==="broadcast"?BROADCAST_NUM:destination,channel:runtime.messageChannel,type:destination==="broadcast"?"broadcast":"direct",text,state:"SENDING",direction:"TX",attempts:1};
  ...
  try {
    ...
    const packet=project.packets.find(p=>p.packetId===packetId); if(packet) message.packetRecordId=packet.id;
...
async function retryMessage(): Promise<void> {
  ...
  if(!message||message.direction!=="TX"||message.state!=="FAILED")return;
  if(runtime.connection!=="CONNECTED")throw new Error("Connect and synchronize a radio before retrying.");
  message.state="SENDING";message.failureReason=undefined;message.failedAt=undefined;message.acknowledgedAt=undefined;message.sentAt=now();message.attempts=Math.max(1,message.attempts)+1;
  scheduleSave();render();
  try {
    ...
    message.packetId=packetId; const packet=project.packets.find(p=>p.packetId===packetId); if(packet) message.packetRecordId=packet.id;
    applyMessageState(message,"ACKNOWLEDGED");
    ...
    const reason=error instanceof Error?error.message:String(error);applyMessageState(message,"FAILED",reason);addTimeline("message retry failed",reason,"MEDIUM",message.type==="direct"?message.to:undefined,"OBSERVED");
  }
  scheduleSave();render();
}
function openMessagePacket(): void {
  const message=project.messages.find(m=>m.id===runtime.selectedMessage); if(!message)return;
  const packet=messagePacket(message); if(!packet)return;
  runtime.selectedPacket=packet.id;runtime.view="packets";render();
}
...
function inputText(id:string): string { return app.querySelector<HTMLInputElement|HTMLTextAreaElement>(`#${id}`)?.value.trim() || ""; }
function inputNumber(id:string): number|undefined { const raw=app.querySelector<HTMLInputElement>(`#${id}`)?.value.trim();if(!raw)return undefined;const value=Number(raw);return Number.isFinite(value)?value:undefined; }
function saveNodeFieldData(): void {
  const node=project.nodes.find(x=>x.num===runtime.selectedNode); if(!node)return;
  ...
  node.notes=meta.notes; addTimeline("node field data updated",`Local field data updated for ${node.shortName}.`,"INFO",node.num,"USER ENTERED");scheduleSave();render();
}
function applySavedNodeView(id:string): void {
  const view=project.nodeTable.savedViews.find(v=>v.id===id); if(!view){project.nodeTable.selectedViewId=undefined;scheduleSave();return;}
  project.nodeTable.filter=safeObject(view.filter);project.nodeTable.visibleColumns=[...view.visibleColumns];project.nodeTable.columnWidths={...view.columnWidths};project.nodeTable.selectedViewId=view.id;scheduleSave();
}
function saveNodeView(): void {
  const requested=inputText("node-view-name"); const current=project.nodeTable.savedViews.find(v=>v.id===project.nodeTable.selectedViewId);
  const name=requested||current?.name||`Node View ${project.nodeTable.savedViews.length+1}`; const stamp=now();
  if(current){current.name=name;current.filter=safeObject(project.nodeTable.filter);current.visibleColumns=[...project.nodeTable.visibleColumns];current.columnWidths={...project.nodeTable.columnWidths};current.updatedAt=stamp;}
  else {const view:SavedNodeView={id:crypto.randomUUID(),name,filter:safeObject(project.nodeTable.filter),visibleColumns:[...project.nodeTable.visibleColumns],columnWidths:{...project.nodeTable.columnWidths},createdAt:stamp,updatedAt:stamp};project.nodeTable.savedViews.push(view);project.nodeTable.selectedViewId=view.id;}
  addTimeline("node view saved",`Saved node inventory view: ${name}.`,"INFO",undefined,"USER ENTERED");scheduleSave();render();
}
function deleteNodeView(): void {
  const id=project.nodeTable.selectedViewId;if(!id)return;const view=project.nodeTable.savedViews.find(v=>v.id===id);project.nodeTable.savedViews=project.nodeTable.savedViews.filter(v=>v.id!==id);project.nodeTable.selectedViewId=undefined;if(view)addTimeline("node view deleted",`Deleted saved node view: ${view.name}.`,"INFO",undefined,"USER ENTERED");scheduleSave();render();
}
function exportNodes(): void {
...
function openSelectedNodeConversation(): void { const node=project.nodes.find(n=>n.num===runtime.selectedNode);if(!node)return;setConversation(`direct:${node.num}`);runtime.view="messages";render(); }
function openSelectedNodePackets(): void { const node=project.nodes.find(n=>n.num===runtime.selectedNode);if(!node)return;runtime.packetSearch="";project.packetLab.filter.search="";project.packetLab.filter.source=String(node.num);project.packetLab.filter.destination="";project.packetLab.filter.portNum="";runtime.view="packets";scheduleSave();render(); }
function evidenceFromNode(): void {
  const node=project.nodes.find(n=>n.num===runtime.selectedNode);if(!node)return;const packets=project.packets.filter(p=>p.direction==="RX"&&p.source===node.num).slice(-20);const rssi=metricStats(nodeMetricSeries(project,node.num,"rssi"));const snr=metricStats(nodeMetricSeries(project,node.num,"snr"));const rec:EvidenceRecord={id:`EVID-${String(project.evidence.length+1).padStart(4,"0")}`,title:`Node state — ${node.shortName}`,time:now(),observation:`${statusForNode(node,settings)}; last heard ${fmtTime(node.lastHeard)}; battery ${pct(node.battery)}; current RSSI ${fixed(node.rssi,0," dBm")}; current SNR ${fixed(node.snr,1," dB")}; retained RF samples ${Math.max(rssi.samples,snr.samples)}.`,nodeNum:node.num,packetIds:packets.map(p=>p.id),notes:"Current node state captured from MESHBOARD; packet IDs reference the most recent retained received observations for this node.",provenance:"OBSERVED"};project.evidence.push(rec);addTimeline("evidence created",rec.title,"INFO",node.num,"USER ENTERED");scheduleSave();runtime.view="evidence";render();
}
function addLog(): void { const t=app.querySelector<HTMLTextAreaElement>("#log-text"),s=app.querySelector<HTMLSelectElement>("#log-node");const text=t?.value.trim();if(!text)return;const nodeNum=s?.value?Number(s.value):undefined;project.logbook.push({id:crypto.randomUUID(),time:now(),text,nodeNum});addTimeline("operator log",text,"INFO",nodeNum,"USER ENTERED");scheduleSave();render(); }
function createSnapshot(): void { const s:NetworkSnapshot={id:crypto.randomUUID(),name:`Snapshot ${project.snapshots.length+1}`,time:now(),radio:safeObject(project.radio),nodes:safeObject(project.nodes),packetCount:project.packets.length,messageCount:project.messages.length,telemetryCount:project.telemetry.length,findings:safeObject(project.findings),config:safeObject(project.config)};project.snapshots.push(s);addTimeline("snapshot created",`${s.name} captured ${s.nodes.length} nodes.`);scheduleSave();render(); }
function currentFilteredPackets(): Project["packets"] {
...
function exportFilteredPackets(): void {
...
function exportSelectedPacket(): void {
  const packet=project.packets.find(p=>p.id===runtime.selectedPacket);if(!packet)return;
  downloadBlob(`${slug(project.name)}-packet-${packet.packetId??packet.id}.json`,"application/json",JSON.stringify(packet,null,2));
}
async function copySelectedPacketHex(): Promise<void> {
  const packet=project.packets.find(p=>p.id===runtime.selectedPacket);if(!packet?.rawHex)return;
...
function exportRfAnalytics(): void {
  ...
  const ids=new Set([...rfPoints(project,"rssi",range,node),...rfPoints(project,"snr",range,node)].map(p=>p.recordId).filter((x):x is string=>!!x));
...
function evidenceFromRf(): void {
  const range=project.rfTelemetry.timeRange,node=project.rfTelemetry.rfNode==="all"?undefined:Number(project.rfTelemetry.rfNode);const rp=rfPoints(project,"rssi",range,node),sp=rfPoints(project,"snr",range,node);const rs=analyticsStats(rp),ss=analyticsStats(sp);
  const packetIds=[...new Set([...rp,...sp].map(p=>p.recordId).filter((x):x is string=>!!x))].slice(-100);const scope=node===undefined?"all observed nodes":nodeName(node);
  const rec:EvidenceRecord={id:`EVID-${String(project.evidence.length+1).padStart(4,"0")}`,title:`RF summary — ${scope}`,time:now(),observation:`${rangeLabel(range)}; RSSI median ${fixed(rs.median,0," dBm")} from ${rs.samples} samples; SNR median ${fixed(ss.median,1," dB")} from ${ss.samples} samples.`,nodeNum:node,packetIds,notes:"Calculated locally from retained packet-associated RSSI/SNR observations. This summary does not imply symmetric link quality or packet-delivery ratio.",provenance:"CALCULATED"};project.evidence.push(rec);addTimeline("evidence created",rec.title,"INFO",node,"USER ENTERED");scheduleSave();runtime.view="evidence";render();
}
function exportTelemetryAnalytics(): void {
  const state=project.rfTelemetry,node=state.telemetryNode==="all"?undefined:Number(state.telemetryNode),descriptor=metricDescriptor(state.telemetryMetric);const points=telemetryPoints(project,state.telemetryMetric,state.timeRange,state.telemetryKind,node);
  const rows=points.map(p=>({time:p.time,node:nodeName(p.nodeNum),nodeNum:p.nodeNum,telemetryType:p.source,metric:descriptor.label,value:p.value,unit:descriptor.unit,recordId:p.recordId,provenance:"OBSERVED"}));downloadBlob(`${slug(project.name)}-telemetry-${descriptor.key}-${state.timeRange}.csv`,"text/csv",csv(rows));
}
function evidenceFromTelemetry(): void {
  const state=project.rfTelemetry,node=state.telemetryNode==="all"?undefined:Number(state.telemetryNode),descriptor=metricDescriptor(state.telemetryMetric);const points=telemetryPoints(project,state.telemetryMetric,state.timeRange,state.telemetryKind,node);const stats=analyticsStats(points);const scope=node===undefined?"all reporting nodes":nodeName(node);
  const rec:EvidenceRecord={id:`EVID-${String(project.evidence.length+1).padStart(4,"0")}`,title:`Telemetry summary — ${descriptor.label}`,time:now(),observation:`${rangeLabel(state.timeRange)}; ${scope}; ${descriptor.label}: latest ${stats.latest===undefined?"—":stats.latest.toFixed(descriptor.decimals)+descriptor.unit}, mean ${stats.mean===undefined?"—":stats.mean.toFixed(descriptor.decimals)+descriptor.unit}, median ${stats.median===undefined?"—":stats.median.toFixed(descriptor.decimals)+descriptor.unit}, ${stats.samples} samples.`,nodeNum:node,packetIds:[],notes:`Calculated from retained ${state.telemetryKind==="all"?"telemetry":state.telemetryKind} records. Missing samples are not interpolated. Source record IDs are preserved in project telemetry history.`,provenance:"CALCULATED"};project.evidence.push(rec);addTimeline("evidence created",rec.title,"INFO",node,"USER ENTERED");scheduleSave();runtime.view="evidence";render();
}

function evidenceFromPacket(): void { const p=project.packets.find(x=>x.id===runtime.selectedPacket);if(!p)return;const rec:EvidenceRecord={id:`EVID-${String(project.evidence.length+1).padStart(4,"0")}`,title:`Packet observation ${p.packetId??p.id.slice(0,8)}`,time:now(),observation:`${p.direction} ${p.portNum||"packet"}; RSSI ${fixed(p.rssi,0," dBm")}; SNR ${fixed(p.snr,1," dB")}; source ${nodeName(p.source)}.`,nodeNum:p.source,packetIds:[p.id],provenance:"OBSERVED"};project.evidence.push(rec);addTimeline("evidence created",rec.title,"INFO",rec.nodeNum,"USER ENTERED");scheduleSave();runtime.view="evidence";render(); }

async function freshStart(): Promise<void> { if(adapter.active) await adapter.disconnect();const old=project.id;project=emptyProject();runtime.connection="DISCONNECTED";runtime.selectedNode=undefined;runtime.selectedPacket=undefined;runtime.selectedMessage=undefined;runtime.selectedConversation="channel:0";runtime.messageDestination="broadcast";runtime.messageChannel=0;runtime.rxCount=0;runtime.txCount=0;await deleteProject(old).catch(()=>{});await saveProject(project);render(); }
function exportProject(): void { downloadBlob(`${slug(project.name)}.meshboard.json`,"application/json",JSON.stringify(project,null,2)); }
async function importProject(file?:File): Promise<void> { if(!file)return;const value=JSON.parse(await file.text()) as Partial<Project>;if(typeof value!=="object"||!value.id||!Array.isArray(value.nodes)||!Array.isArray(value.packets)||!Array.isArray(value.messages))throw new Error("This JSON file is not a valid MESHBOARD project.");if(adapter.active)await adapter.disconnect();project=normalizeProject(value);project.updatedAt=now();runtime.selectedConversation=project.messaging.lastConversation||"channel:0";setConversation(runtime.selectedConversation,false);runtime.selectedMessage=undefined;await saveProject(project);evaluateFindings();render(); }
...
function printReport(): void {
  const range=project.rfTelemetry.timeRange;const rfNode=project.rfTelemetry.rfNode==="all"?undefined:Number(project.rfTelemetry.rfNode);const rssi=analyticsStats(rfPoints(project,"rssi",range,rfNode)),snr=analyticsStats(rfPoints(project,"snr",range,rfNode));
...
async function init(): Promise<void> {
  ...
  if(initialConversation.kind==="channel"){runtime.messageDestination="broadcast";runtime.messageChannel=initialConversation.channel;}
  else {runtime.messageDestination=initialConversation.peer; const recent=[...conversationMessages(runtime.selectedConversation)].reverse()[0]; if(recent) runtime.messageChannel=recent.channel;}
  evaluateFindings();
  const est=await estimateStorage().catch(()=>undefined); storageText=est?.usage!==undefined?`${(est.usage/1024/1024).toFixed(1)} MB used${est.quota?` / ${(est.quota/1024/1024).toFixed(0)} MB quota`:""}`:"Available";
  runtime.sdkState="Ready"; await configureServiceWorker(); render();
}
