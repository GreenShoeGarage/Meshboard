import type { AppSettings, NodeColumnId, NodeFilterState, NodeMetadata, NodeObservation, NodeRecord, NodeStatus, Project } from "./models";
...
export const NODE_COLUMNS: Array<{ id: NodeColumnId; label: string; defaultWidth: number }> = [
...
export function statusForNode(node: NodeRecord, settings: AppSettings, time = Date.now()): NodeStatus {
  if (!node.lastHeard) return "UNKNOWN";
  ...
  if (minutes <= settings.activeMinutes) return "ACTIVE";
  if (minutes <= settings.staleMinutes) return "RECENT";
  if (minutes <= settings.lostMinutes) return "STALE";
  return "LOST";
}

export function metadataFor(project: Project, nodeNum: number): NodeMetadata {
...
export function appendNodeObservation(project: Project, observation: NodeObservation, limit: number): void {
  ...
  if (observation.kind === "NODEDB" || observation.kind === "TELEMETRY") {
    let previous: NodeObservation | undefined;
    const floor = Math.max(0, project.nodeObservations.length - 1000);
    for (let i=project.nodeObservations.length-1;i>=floor;i--) { const candidate=project.nodeObservations[i]; if(candidate?.nodeNum===observation.nodeNum&&candidate.kind===observation.kind){previous=candidate;break;} }
    const comparable = (x: NodeObservation | undefined) => x ? JSON.stringify({
    ...
    if (previous && comparable(previous) === comparable(observation) && ageMs < 60_000) return;
  }
  project.nodeObservations.push(observation);
  const cap = Math.max(1000, limit);
  if (project.nodeObservations.length > cap) project.nodeObservations.splice(0, project.nodeObservations.length - cap);
}

export function nodeMetricSeries(project: Project, nodeNum: number, metric: "rssi"|"snr"|"battery"|"voltage"): MetricPoint[] {
  const points: MetricPoint[] = [];
  for (const obs of project.nodeObservations) {
    if (obs.nodeNum !== nodeNum) continue;
    const value = obs[metric];
    if (typeof value === "number" && Number.isFinite(value)) points.push({ time: obs.time, value });
  }
  points.sort((a,b)=>new Date(a.time).getTime()-new Date(b.time).getTime());
  return points;
}

export function metricStats(points: MetricPoint[]): MetricStats {
  if (!points.length) return { samples: 0 };
...
function textValue(node: NodeRecord, meta: NodeMetadata | undefined, id: NodeColumnId, settings: AppSettings): string | number {
  switch (id) {
...
export function filteredSortedNodes(project: Project, settings: AppSettings, filter: NodeFilterState): NodeRecord[] {
  ...
  const rows = project.nodes.filter((node) => {
    ...
    if (filter.status !== "all" && statusForNode(node, settings) !== filter.status) return false;
    if (filter.role && node.role !== filter.role) return false;
    if (filter.hardware && node.hardware !== filter.hardware) return false;
    if (filter.favoritesOnly && !node.favorite) return false;
    return true;
  });
...
export function svgSparkline(points: MetricPoint[], label: string, unit: string, decimals = 1): string {
  if (!points.length) return `<div class="history-empty">No ${label.toLowerCase()} observations recorded.</div>`;
  const W=520,H=130,padX=32,padY=20;
  const values=points.map((x)=>x.value); let min=Math.min(...values),max=Math.max(...values);
