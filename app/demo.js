...
export function createDemoProject() {
    ...
    p.packets = p.messages.filter(m => m.packetId !== undefined).map((m) => {
    ...
    for (const m of p.messages) {
        const packet = p.packets.find(x => x.packetId === m.packetId);
        if (packet)
            m.packetRecordId = packet.id;
    }
    for (let i = 0; i < 96; i++) {
    ...
    for (const [idx, node] of p.nodes.entries()) {
        for (let k = 0; k < 8; k++) {
            ...
            p.telemetry.push({ id: crypto.randomUUID(), nodeNum: node.num, time: ago((7 - k) * 28 + idx), kind: "deviceMetrics",
                values: { batteryLevel: battery, voltage: (node.voltage ?? 3.9) - (7 - k) * 0.005, channelUtilization: 3.5 + ((idx + k) % 10) * 0.9, airUtilTx: 0.5 + ((idx + k) % 5) * 0.35, uptimeSeconds: 21800 + k * 1680 }, provenance: "OBSERVED" });
        }
    }
    const weather = 0x93ab1204;
    for (let k = 0; k < 12; k++)
        p.telemetry.push({ id: crypto.randomUUID(), nodeNum: weather, time: ago((11 - k) * 32 + 5), kind: "environmentMetrics", values: { temperature: 16.8 - k * 0.08 + Math.sin(k) * 0.4, relativeHumidity: 61 + k * 0.6, barometricPressure: 1014.2 - k * 0.18, gasResistance: 128.5 + k * 1.3, lux: Math.max(4, 680 - k * 48), windDirection: (235 + k * 7) % 360, windSpeed: 2.1 + (k % 4) * 0.35, windGust: 3.5 + (k % 3) * 0.6, rainfall1h: k > 8 ? 0.4 : 0, rainfall24h: 1.8 }, provenance: "OBSERVED" });
    for (let k = 0; k < 8; k++)
        p.telemetry.push({ id: crypto.randomUUID(), nodeNum: weather, time: ago((7 - k) * 45 + 8), kind: "airQualityMetrics", values: { pm10Standard: 7 + (k % 3), pm25Standard: 12 + (k % 5), pm100Standard: 18 + (k % 6), co2: 468 + k * 5, pmVocIdx: 72 + k * 1.8, pmNoxIdx: 18 + k * 0.7, pmTemperature: 17.1 + k * 0.04, pmHumidity: 62 + k * 0.5 }, provenance: "OBSERVED" });
    for (let k = 0; k < 10; k++)
        p.telemetry.push({ id: crypto.randomUUID(), nodeNum: base, time: ago((9 - k) * 20 + 3), kind: "localStats", values: { uptimeSeconds: 18400 + k * 1200, channelUtilization: 5.2 + k * 0.45, airUtilTx: 0.8 + k * 0.09, numPacketsTx: 410 + k * 28, numPacketsRx: 1220 + k * 74, numPacketsRxBad: 12 + k, numOnlineNodes: 10 + (k % 4), numTotalNodes: 14, numRxDupe: 65 + k * 7, numTxRelay: 18 + k * 2, numTxRelayCanceled: 24 + k * 3, heapTotalBytes: 327680, heapFreeBytes: 171000 - k * 1150, numTxDropped: k > 7 ? k - 7 : 0, noiseFloor: -116 + (k % 3) }, provenance: "OBSERVED" });
    for (let k = 0; k < 7; k++)
        p.telemetry.push({ id: crypto.randomUUID(), nodeNum: base, time: ago((6 - k) * 35 + 6), kind: "powerMetrics", values: { ch1Voltage: 5.05 - k * 0.006, ch1Current: 0.31 + k * 0.012, ch2Voltage: 4.12 - k * 0.008, ch2Current: 0.18 + k * 0.009 }, provenance: "OBSERVED" });
    for (let k = 0; k < 6; k++)
        p.telemetry.push({ id: crypto.randomUUID(), nodeNum: 0x93ab1206, time: ago((5 - k) * 40 + 4), kind: "trafficManagementStats", values: { packetsInspected: 730 + k * 96, positionDedupDrops: 22 + k * 3, nodeinfoCacheHits: 80 + k * 5, rateLimitDrops: k > 3 ? k - 3 : 0, unknownPacketDrops: 4 + k, hopExhaustedPackets: 2 + k, routerHopsPreserved: 40 + k * 6 }, provenance: "OBSERVED" });
    p.positions = p.nodes.filter(x => x.latitude && x.longitude).map((node, i) => ({
    ...
    const nodeDbObs = p.nodes.map(node => ({ id: crypto.randomUUID(), nodeNum: node.num, time: node.lastHeard || ago(1), kind: "NODEDB", lastHeard: node.lastHeard, battery: node.battery, voltage: node.voltage, rssi: node.rssi, snr: node.snr, hops: node.hops, latitude: node.latitude, longitude: node.longitude, altitude: node.altitude, channelUtilization: node.channelUtilization, airUtilTx: node.airUtilTx, provenance: "OBSERVED" }));
    const packetObs = p.packets.filter(packet => packet.direction === "RX" && packet.source !== undefined && (packet.rssi !== undefined || packet.snr !== undefined)).map(packet => ({ id: crypto.randomUUID(), nodeNum: packet.source, time: packet.time, kind: "PACKET", rssi: packet.rssi, snr: packet.snr, packetRecordId: packet.id, provenance: "OBSERVED" }));
    const telemetryObs = p.telemetry.map(reading => ({ id: crypto.randomUUID(), nodeNum: reading.nodeNum, time: reading.time, kind: "TELEMETRY", battery: typeof reading.values.batteryLevel === "number" ? reading.values.batteryLevel : undefined, voltage: typeof reading.values.voltage === "number" ? reading.values.voltage : undefined, channelUtilization: typeof reading.values.channelUtilization === "number" ? reading.values.channelUtilization : undefined, airUtilTx: typeof reading.values.airUtilTx === "number" ? reading.values.airUtilTx : undefined, telemetryRecordId: reading.id, provenance: "OBSERVED" })).filter(obs => [obs.battery, obs.voltage, obs.channelUtilization, obs.airUtilTx].some(v => v !== undefined));
    const positionObs = p.positions.map(position => ({ id: crypto.randomUUID(), nodeNum: position.nodeNum, time: position.time, kind: "POSITION", latitude: position.latitude, longitude: position.longitude, altitude: position.altitude, positionRecordId: position.id, provenance: "OBSERVED" }));
    p.nodeObservations = [...nodeDbObs, ...packetObs, ...telemetryObs, ...positionObs].sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());
    const attentionView = { id: crypto.randomUUID(), name: "Stale nodes", filter: { search: "", status: "STALE", role: "", hardware: "", favoritesOnly: false, sortBy: "lastHeard", sortDir: "asc" }, visibleColumns: ["status", "node", "role", "lastHeard", "battery", "rssi", "snr", "field"], columnWidths: { node: 190, lastHeard: 155, field: 160 }, createdAt: ago(1), updatedAt: ago(1) };
    const relayView = { id: crypto.randomUUID(), name: "Relays / infrastructure", filter: { search: "", status: "all", role: "ROUTER_CLIENT", hardware: "", favoritesOnly: false, sortBy: "node", sortDir: "asc" }, visibleColumns: ["status", "node", "hardware", "role", "lastHeard", "battery", "rssi", "snr", "position", "field"], columnWidths: { node: 190, position: 210, field: 170 }, createdAt: ago(1), updatedAt: ago(1) };
