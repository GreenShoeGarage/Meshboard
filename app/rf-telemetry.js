...
function camelize(key) { return key.replace(/_([a-z0-9])/g, (_, c) => String(c).toUpperCase()); }
export function canonicalTelemetryKey(key) { return camelize(key); }
export function metricDescriptor(key) {
    const canonical = canonicalTelemetryKey(key);
    const meta = METRIC_META[canonical];
    if (meta)
        return { key: canonical, ...meta };
    const label = canonical.replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/^./, c => c.toUpperCase());
    if (/voltage$/i.test(canonical))
        return { key: canonical, label, unit: " V", decimals: 2, group: "POWER" };
    if (/current$/i.test(canonical))
        return { key: canonical, label, unit: " A", decimals: 3, group: "POWER" };
    if (/temperature/i.test(canonical))
        return { key: canonical, label, unit: " °C", decimals: 1, group: "ENVIRONMENT" };
    if (/humidity/i.test(canonical))
        return { key: canonical, label, unit: "%", decimals: 1, group: "ENVIRONMENT" };
    if (/bytes$/i.test(canonical))
        return { key: canonical, label, unit: " B", decimals: 0, group: "HOST" };
    return { key: canonical, label, unit: "", decimals: 2, group: "OTHER" };
}
export function rangeStart(range, anchor = Date.now()) {
...
export function inRange(time, range, anchor = Date.now()) { const start = rangeStart(range, anchor); if (start === undefined)
    return true; const t = new Date(time).getTime(); return Number.isFinite(t) && t >= start && t <= anchor; }
export function rangeLabel(range) { return { "1h": "LAST HOUR", "6h": "LAST 6 HOURS", "24h": "LAST 24 HOURS", "7d": "LAST 7 DAYS", "all": "ALL RETAINED" }[range]; }
export function analyticsStats(points) {
    if (!points.length)
        return { samples: 0 };
...
export function rfPoints(project, metric, range, nodeNum) {
    return project.packets.filter(p => p.direction === "RX" && p.source !== undefined && typeof p[metric] === "number" && inRange(p.time, range)).filter(p => nodeNum === undefined || p.source === nodeNum).map(p => ({ time: p.time, value: p[metric], nodeNum: p.source, recordId: p.id, source: "PACKET" }));
}
export function rfNodeStats(project, range) {
    const map = new Map();
    for (const p of project.packets) {
        if (p.direction !== "RX" || p.source === undefined || !inRange(p.time, range))
            continue;
        const row = map.get(p.source) ?? { rssi: [], snr: [], times: [] };
        if (typeof p.rssi === "number")
            row.rssi.push({ time: p.time, value: p.rssi, nodeNum: p.source, recordId: p.id });
        if (typeof p.snr === "number")
            row.snr.push({ time: p.time, value: p.snr, nodeNum: p.source, recordId: p.id });
        row.times.push(p.time);
...
export function packetRateBins(project, range, maxBins = 36) {
    let packets = project.packets.filter(p => inRange(p.time, range));
    if (!packets.length)
        return [];
    const times = packets.map(p => new Date(p.time).getTime()).filter(Number.isFinite);
    if (!times.length)
        return [];
    const end = Math.max(...times, Date.now());
    const configuredStart = rangeStart(range, end);
    const start = configuredStart ?? Math.min(...times);
    const span = Math.max(60000, end - start);
    const natural = range === "1h" ? 5 * 60000 : range === "6h" ? 30 * 60000 : range === "24h" ? 2 * 3600000 : range === "7d" ? 12 * 3600000 : Math.max(60000, Math.ceil(span / maxBins / 60000) * 60000);
    const bucket = Math.max(60000, natural);
    const bins = Math.max(1, Math.min(maxBins, Math.ceil(span / bucket)));
    const actualStart = end - bins * bucket;
    const rows = Array.from({ length: bins }, (_, i) => ({ start: actualStart + i * bucket, end: actualStart + (i + 1) * bucket, rx: 0, tx: 0 }));
    for (const p of packets) {
        const t = new Date(p.time).getTime();
        if (t < actualStart || t > end)
            continue;
        const i = Math.min(rows.length - 1, Math.floor((t - actualStart) / bucket));
        const row = rows[i];
        if (!row)
            continue;
        if (p.direction === "RX")
...
function flattenNumeric(value, prefix = "", out = {}) {
    if (!value || typeof value !== "object")
        return out;
    for (const [raw, v] of Object.entries(value)) {
        const key = canonicalTelemetryKey(raw);
...
export function telemetryNumericValues(record) { return flattenNumeric(record.values); }
export function telemetryMetricKeys(project, range, kind = "all", nodeNum) {
    const byKey = new Map();
    for (const t of project.telemetry) {
        if (!inRange(t.time, range) || kind !== "all" && t.kind !== kind || nodeNum !== undefined && t.nodeNum !== nodeNum)
            continue;
        for (const path of Object.keys(telemetryNumericValues(t))) {
            const descriptor = metricDescriptor(path.split(".").at(-1) ?? path);
...
export function telemetryPoints(project, metricKey, range, kind = "all", nodeNum) {
    const canonical = canonicalTelemetryKey(metricKey);
    const rows = [];
    for (const t of project.telemetry) {
        if (!inRange(t.time, range) || kind !== "all" && t.kind !== kind || nodeNum !== undefined && t.nodeNum !== nodeNum)
            continue;
        const values = telemetryNumericValues(t);
        for (const [path, value] of Object.entries(values)) {
            if (canonicalTelemetryKey(path.split(".").at(-1) ?? path) === canonical)
                rows.push({ time: t.time, value, nodeNum: t.nodeNum, recordId: t.id, source: t.kind });
        }
    }
    return rows.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());
}
export function telemetryKinds(project) { return [...new Set(project.telemetry.map(t => t.kind))].sort(); }
export function svgTimeSeries(points, label, unit, decimals = 1) {
    if (!points.length)
        return `<div class="analytics-empty">No ${label.toLowerCase()} observations in this window.</div>`;
    const W = 760, H = 210, l = 48, r = 18, t = 22, b = 30;
...
export function svgHistogram(values, label, unit, bins = 14, decimals = 1) {
    if (!values.length)
        return `<div class="analytics-empty">No ${label.toLowerCase()} samples in this window.</div>`;
    let min = Math.min(...values), max = Math.max(...values);
    if (min === max) {
        min -= 1;
        max += 1;
    ...
    const W = 760, H = 200, l = 48, r = 18, t = 18, b = 32;
    const bw = (W - l - r) / bins;
    const bars = counts.map((c, i) => { const h = (c / peak) * (H - t - b); return `<rect x="${(l + i * bw + 1).toFixed(1)}" y="${(H - b - h).toFixed(1)}" width="${Math.max(1, bw - 2).toFixed(1)}" height="${h.toFixed(1)}"><title>${c} samples</title></rect>`; }).join("");
    return `<svg class="analytics-chart histogram" viewBox="0 0 ${W} ${H}" role="img" aria-label="${label} distribution"><line x1="${l}" y1="${H - b}" x2="${W - r}" y2="${H - b}"/>${bars}<text x="${l}" y="${H - 8}">${min.toFixed(decimals)}${unit}</text><text x="${W - r}" y="${H - 8}" text-anchor="end">${max.toFixed(decimals)}${unit}</text><text x="${l}" y="12">peak bin ${peak}</text></svg>`;
}
export function svgPacketRate(bins) { if (!bins.length)
    return '<div class="analytics-empty">No packets in this window.</div>'; const W = 760, H = 210, l = 48, r = 18, t = 20, b = 32; const peak = Math.max(...bins.map(x => x.total), 1); const bw = (W - l - r) / bins.length; const bars = bins.map((x, i) => { const rxh = (x.rx / peak) * (H - t - b), txh = (x.tx / peak) * (H - t - b); return `<g><rect class="rx-bar" x="${(l + i * bw + 1).toFixed(1)}" y="${(H - b - rxh).toFixed(1)}" width="${Math.max(1, (bw - 3) / 2).toFixed(1)}" height="${rxh.toFixed(1)}"><title>RX ${x.rx}</title></rect><rect class="tx-bar" x="${(l + i * bw + 1 + (bw - 3) / 2).toFixed(1)}" y="${(H - b - txh).toFixed(1)}" width="${Math.max(1, (bw - 3) / 2).toFixed(1)}" height="${txh.toFixed(1)}"><title>TX ${x.tx}</title></rect></g>`; }).join(""); return `<svg class="analytics-chart packet-rate" viewBox="0 0 ${W} ${H}" role="img" aria-label="Packet activity over time"><line x1="${l}" y1="${H - b}" x2="${W - r}" y2="${H - b}"/>${bars}<text x="4" y="${t + 4}">${peak}</text><text x="${l}" y="${H - 8}">${new Date(bins[0].start).toLocaleString()}</text><text x="${W - r}" y="${H - 8}" text-anchor="end">${new Date(bins.at(-1).end).toLocaleString()}</text></svg>`; }
